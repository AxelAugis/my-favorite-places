...
export const ResultsWrapper = (props: ResultsWrapperProps) => {
    return (
        <div className={`absolute top-full left-0 w-full mt-1 bg-[#0A3633] rounded-lg shadow-lg border border-slate-600/50 p-4 animate-fade-in z-20 ${!props.isOpen && "hidden"}`}>
            ...
            {props.results !== null && props.results.length > 0 && (
                <ul className={`flex flex-col gap-y-2`}>
                    {props.results.map((result) => (
                        <li key={result.id} className={`p-2 bg-[#0A3633] rounded-md border border-slate-600/50 hover:border-[#E0F6EB] transition-colors cursor-pointer`}>
                            {result.name}
                        </li>
                    ))}
                </ul>
            ...
            {props.results !== null && props.results.length === 0 && props.loadingFallback && (
                <p className="text-slate-400">Aucun favori local correspondant.</p>
            )}
            ...
            {props.fallback && props.fallback.length > 0 && (
                <ul className={`flex flex-col gap-y-2`}>
                    {props.fallback.map((item, idx) => (
                        <li key={`${item.name}-${idx}`} className={`p-2 bg-[#E0F6EB] rounded-md border border-slate-600/50 hover:border-[#0A3633] text-[#0A3633] transition-colors cursor-pointer`}>
